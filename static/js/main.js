...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 系统告警实时监听功能
    if (document.querySelector('.alert-monitoring') || document.querySelector('.admin-dashboard')) {
        initAlertMonitoring();
    }
});
...
function initAlertMonitoring() {
    ...
    // 开始轮询检查新告警
    setInterval(checkForNewAlerts, POLLING_INTERVAL);
    
    // 检查新告警的函数
    function checkForNewAlerts() {
        fetch('/api/check_new_alerts/?since=' + encodeURIComponent(lastCheckedTimestamp))
                ...
                if (data.status === 'success') {
                    ...
                    if (data.alerts && data.alerts.length > 0) {
                        data.alerts.forEach(alert => {
                            if (alert.alert_type === '跌倒监测' || alert.alert_type === '紧急求助') {
            ...
            .catch(error => console.error('检查告警出错:', error));
    }
    ...
    function showEmergencyAlert(alert) {
        ...
        alertModal.innerHTML = `
            <div class="emergency-alert-content">
                ...
                <div class="emergency-body">
                    <p><strong>老人:</strong> ${alert.elder_name}</p>
                ...
                <div class="emergency-footer">
                    <button class="btn btn-primary emergency-action" data-alert-id="${alert.id}" data-action="process">立即处理</button>
                    <button class="btn btn-secondary emergency-close">关闭</button>
                </div>
            </div>
        `;
        ...
        // 添加闪烁效果
        alertModal.classList.add('flashing');
        
        // 添加事件监听
        alertModal.querySelector('.emergency-close').addEventListener('click', function() {
            alertModal.classList.add('closing');
            setTimeout(() => alertModal.remove(), 500);
        });
        
        // 处理按钮点击事件
        alertModal.querySelector('.emergency-action').addEventListener('click', function(e) {
    ...
    function processAlert(alertId) {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                if (alertRow) {
                    alertRow.querySelector('.alert-status').innerHTML = 
                        '<span class="badge bg-success">已处理</span>';
                }
            }
        })
        .catch(error => console.error('处理告警出错:', error));
    }
    ...
    function updateAlertTable(alerts) {
        ...
        alerts.forEach(alert => {
            ...
            // 如果是紧急告警，添加特殊样式
            if (alert.alert_type === '跌倒监测' || alert.alert_type === '紧急求助') {
                newRow.classList.add('emergency-alert-row');
            }
            
            // 设置行内容
            newRow.innerHTML = `
                <td>${new Date(alert.timestamp).toLocaleString()}</td>
                <td>${alert.elder_name}</td>
                ...
                <td>
                    <button class="btn btn-sm btn-primary process-alert-btn" data-alert-id="${alert.id}">
                        处理
                    </button>
                </td>
            `;
            ...
            // 添加闪烁效果
            newRow.classList.add('flashing-row');
            setTimeout(() => newRow.classList.remove('flashing-row'), 10000);
        });
        
        // 添加事件监听
        document.querySelectorAll('.process-alert-btn').forEach(btn => {
    ...
    function playAlertSound() {
        const alertSound = new Audio('/static/sounds/emergency_alert.mp3');
        alertSound.play().catch(e => console.log('无法播放告警声音:', e));
    }
